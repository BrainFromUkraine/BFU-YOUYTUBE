...
# YOUTUBE API
# IMPORTANT: paste your own API key here. Do not publish this file with your real key.
YOUTUBE_API_KEY = " "
YOUTUBE_CHANNEL_ID = " "
YOUTUBE_UPDATE_MS = 60000
...
FONT_UA = {
    ...
    "Ї": ["01010","00000","11111","00100","00100","00100","11111"],
    "Й": ["00100","00000","10001","10011","10101","11001","10001"],
...
# OLD WIFI LIST/PASSWORD FUNCTIONS LEFT IN PLACE, BUT NOT USED NOW.
...
# NEW WIFI SETUP PORTAL FUNCTIONS
...
def rotate_right():
    ...
    if screen_mode == "MENU":
        ...
        if len(wifi_networks) > 0:
            old = wifi_selected
            wifi_selected += 1
            wifi_selected = wifi_selected % len(wifi_networks)
            update_wifi_selection(old, wifi_selected)
    elif screen_mode == "WIFI_PASSWORD":
        wifi_char_index += 1
        wifi_char_index = wifi_char_index % len(password_chars)
        draw_char_picker()
